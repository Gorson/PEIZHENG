...
#import "SDBWebImageManager.h"
#import "SDBImageCache.h"
#import "SDBWebImageDownloader.h"
#import "UrlImageView.h"
static SDBWebImageManager *instance;

@implementation SDBWebImageManager
...
+ (id)sharedManager
{
    if (instance == nil)
    {
        instance = [[SDBWebImageManager alloc] init];
    }
...
{
    return [[SDBImageCache sharedImageCache] imageFromKey:[url absoluteString]];
}
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDBWebImageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed lowPriority:(BOOL)lowPriority
{
    ...
    [[SDBImageCache sharedImageCache] queryDiskCacheForKey:[url absoluteString] delegate:self userInfo:info];
}
...
- (void)cancelForDelegate:(id<SDBWebImageManagerDelegate>)delegate
{
    ...
    {
        SDBWebImageDownloader *downloader = [downloaders objectAtIndex:idx] ;
...
- (void)imageCache:(SDBImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    SDBWebImageDownloader *downloader = [downloaderForURL objectForKey:url];

    if (!downloader)
    {
        downloader = [SDBWebImageDownloader downloaderWithURL:url delegate:self userInfo:nil lowPriority:lowPriority];
...
#pragma mark SDBWebImageDownloaderDelegate
...
- (void)imageDownloader:(SDBWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        SDBWebImageDownloader *aDownloader = [downloaders objectAtIndex:uidx];
        if (aDownloader == downloader)
        {
            id<SDBWebImageManagerDelegate> delegate = [downloadDelegates objectAtIndex:uidx];
    ...
    if (image)
    {
        ...
        [[SDBImageCache sharedImageCache] storeImage:image
