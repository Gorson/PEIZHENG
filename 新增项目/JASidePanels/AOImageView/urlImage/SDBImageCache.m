...
static SDBImageCache *instance;

@implementation SDBImageCache
...
#pragma mark SDBImageCache (class methods)

+ (SDBImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[SDBImageCache alloc] init];
    }

    return instance;
}

#pragma mark SDBImageCache (private)
...
- (void)storeKeyWithDataToDisk:(NSArray *)keyAndData
{
    ...
    else
    {
        ...
        if (image)
        {
            ...
            NSDBata* jpegData = [NSBitmapImageRep representationOfImageRepsInArray: representations usingType: NSJPEGFileType properties:nil];
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    id <SDBImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
    ...
    [SDimageView setImageWithURL:[NSURL URLWithString:key]];
    if (SDimageView.image)
    {
        [mutableArguments setObject:SDimageView.image forKey:@"image"];
    }
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <SDBImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    if (!delegate)
    {
        return;
